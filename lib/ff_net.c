...
static uint32_t
ff_ptype_l3_ip6(uint8_t ip6_proto)
{
...
static uint32_t
ff_ptype_l3_ip(uint8_t ipv_ihl)
{
...
static uint32_t
ff_ptype_inner_l3_ip6(uint8_t ip6_proto)
{
...
static uint32_t
ff_ptype_inner_l3_ip(uint8_t ipv_ihl)
{
...
static uint32_t
ff_ptype_inner_l4(uint8_t proto)
{
...
static uint32_t
ff_ptype_tunnel(uint16_t *proto, const void *data, uint16_t data_len, 
	uint32_t *off)
{
	switch (*proto) {
	case IPPROTO_GRE: {
		...
		uint16_t flags;
		gh = (sizeof(*gh) + *off <= data_len) ? (data + *off) : NULL;
		if (unlikely(gh == NULL))
			return 0;
...
#define MAX_EXT_HDRS 5
	for (i = 0; i < MAX_EXT_HDRS; i++) {
		...
		case IPPROTO_DSTOPTS:
			xh = (sizeof(*xh) + *off <= data_len) ? (data + *off) : NULL;
			if (xh == NULL)
				return -1;
			*off += (xh->len + 1) * 8;
			proto = xh->next_hdr;
...
{
	struct ff_net_hdr_lens local_hdr_lens;
	...
	eh = (sizeof(*eh) + off <= data_len) ? data : NULL;
	if (unlikely(eh == NULL))
		return 0;
	proto = eh->ether_type;
	off = sizeof(*eh);
	...
	if (proto == rte_cpu_to_be_16(RTE_ETHER_TYPE_VLAN)) {
		...
		vh = (sizeof(*vh) + off <= data_len) ? (data + off) : NULL;
		if (unlikely(vh == NULL))
			return pkt_type;
		off += sizeof(*vh);
		...
		vh = ((sizeof(*vh) * 2) + off <= data_len) ? (data + off + sizeof(*vh)) : NULL;
		if (unlikely(vh == NULL))
			return pkt_type;
		off += 2 * sizeof(*vh);
...
#define MAX_MPLS_HDR 5
		for (i = 0; i < MAX_MPLS_HDR; i++) {
			mh = (((i + 1) * sizeof(*mh)) + off <= data_len) ? (data + off + (i * sizeof(*mh))) : NULL;
			if (unlikely(mh == NULL))
				return pkt_type;
		}
		if (i == MAX_MPLS_HDR)
...
l3:
	...
	if (proto == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4)) {
		...
		ip4h = (sizeof(*ip4h) + off <= data_len) ? (data + off) : NULL;
		if (unlikely(ip4h == NULL))
			return pkt_type;

		pkt_type |= ff_ptype_l3_ip(ip4h->version_ihl);
		...
		pkt_type |= ff_ptype_l4(proto);
	} else if (proto == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV6)) {
		const struct rte_ipv6_hdr *ip6h;
		int frag = 0;
		
		ip6h = (sizeof(*ip6h) + off <= data_len) ? (data + off) : NULL;
		...
		off += hdr_lens->l3_len;
		pkt_type |= ff_ptype_l3_ip6(proto);
		if ((pkt_type & RTE_PTYPE_L3_MASK) == RTE_PTYPE_L3_IPV6_EXT) {
			ret = ff_net_skip_ip6_ext(proto, data, data_len, &off, &frag);
			if (ret < 0)
				return pkt_type;
			proto = ret;
	...
	if ((pkt_type & RTE_PTYPE_L4_MASK) == RTE_PTYPE_L4_UDP) {
		...
		th = (sizeof(*th) + off <= data_len) ? (data + off) : NULL;
		if (unlikely(th == NULL))
		...
		pkt_type |= ff_ptype_tunnel(&proto, data, data_len, &off);
	...
	if (proto == rte_cpu_to_be_16(RTE_ETHER_TYPE_TEB)) {
		eh = (sizeof(*eh) + off <= data_len) ? (data + off) : NULL;
		if (unlikely(eh == NULL))
			return pkt_type;
	...
	if (proto == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4)) {
		...
		pkt_type |= ff_ptype_inner_l3_ip(ip4h->version_ihl);
		...
		proto = ip4h->next_proto_id;
		pkt_type |= ff_ptype_inner_l4(proto);
	} else if (proto == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV6)) {
		const struct rte_ipv6_hdr *ip6h;
		int frag = 0;
		...
		off += hdr_lens->inner_l3_len;
		pkt_type |= ff_ptype_inner_l3_ip6(proto);
